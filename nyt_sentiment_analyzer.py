...
def sentiment_analysis_pipe(directory):
    ...
    # define candidate collection: candidate_list
    candidate_list = [
        ('Sanders', 'Senator Bernie Sanders'),
        ('Trump', 'President Donald Trump'),
        ('Bloomberg', 'Mayor Michael Bloomberg')
    ]

    # plot Time Series Line plot
    time_series_line_viz(
        df=predictions_df[predictions_df['text'].str.contains(candidate_list[1][0], case=False)],
        date_index='pub_date',
        pd_series='predictions',
        plot_title='N.Y. Times Articles Avg. Daily Sentiment for {}'.format(candidate_list[1][1])
    )
