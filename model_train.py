...
from model_utils.model_eval import (model_training_metrics,
                                    num_feature_importance, model_random_hyper_tune,
...
def model_train_pipeline(directory):
    """
    ...
    time_series_line_viz(
        df=article_df[article_df['text'].str.contains('Sanders', case=False)],
        date_index='pub_date',
        pd_series='polarity',
    ...
    text_models = [
        Pipeline([('vectorizer', CountVectorizer(ngram_range=(1, 3), stop_words=my_stopwords)),
                  ('scaler', StandardScaler(with_mean=False)),
                  ('dim_red', SelectKBest(chi2, k=300)),
                  ('clf', OneVsRestClassifier(MultinomialNB()))
                  ]),
        Pipeline([('vectorizer', TfidfVectorizer(ngram_range=(1, 3), stop_words=my_stopwords)),
                  ('scaler', StandardScaler(with_mean=False)),
                  ('dim_red', SelectKBest(chi2, k=300)),
                  ('clf', OneVsRestClassifier(MultinomialNB()))
                  ]),
    ...
    text_pipe, text_cv_results_df, text_model_params = model_random_hyper_tune(
        df=model_df,
    ...
    text_feature_importance(
        df=model_df[model_df['sentiment_label'] == 'positive'],
    ...
    num_pipe, num_cv_results_df = model_random_hyper_tune(
        ...
        param_grid={'clf__estimator__booster': ['gbtree', 'gblinear', 'dart'],
                    'clf__estimator__colsample_bytree': [0.3, 0.7],
                    'clf__estimator__n_estimators': [100, 200, 300],
                    'clf__estimator__max_depth': [3, 6, 10, 20],
...
if __name__ == '__main__':
    model_train_pipeline(directory=r'.\data')
